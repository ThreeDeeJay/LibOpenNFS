...
using namespace LibOpenNFS::NFS3::Loader;

static CarData ParseFCEModels(const FceFile &fceFile);
static std::vector<OpenNFS::TrackBlock> ParseTRKModels(const FrdFile &frdFile, Track &track);
static std::vector<VirtualRoad> ParseVirtualRoad(const ColFile &colFile);
static std::vector<Entity> ParseCOLModels(const ColFile &colFile, Track &track);
...
std::shared_ptr<Car> LoadCar(const std::string &carBasePath)
{
    ...
    CarData carData = ParseFCEModels(fceFile);
...
Track LoadTrack(const std::string &trackBasePath)
{
    ...
    auto trackNameIt = std::find_if(std::filesystem::directory_iterator{p}, std::filesystem::directory_iterator{}, [](const auto& it){
        return it.path().extension().compare(".FRD") == 0;
    });

    ASSERT(trackNameIt != std::filesystem::directory_iterator{}, "Could not found any FRD file in: " << trackBasePath);
    track.name = p.filename().string();

    std::string trackNameStripped = trackNameIt->path().stem().string();

    std::string frdPath, colPath, canPath, hrzPath, binPath;
    
    std::string strip = "k0";
    
    size_t pos = trackNameStripped.find(strip);
    ...
    track.trackBlocks     = ParseTRKModels(frdFile, track);
    track.globalObjects   = ParseCOLModels(colFile, track);
    track.virtualRoad     = ParseVirtualRoad(colFile);
...
CarData ParseFCEModels(const FceFile &fceFile)
{
...
std::vector<OpenNFS::TrackBlock> ParseTRKModels(const FrdFile &frdFile, Track &track)
{
...
std::vector<VirtualRoad> ParseVirtualRoad(const ColFile &colFile)
{
...
std::vector<Entity> ParseCOLModels(const ColFile &colFile, Track &track)
{
