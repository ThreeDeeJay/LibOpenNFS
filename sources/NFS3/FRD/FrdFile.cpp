...
bool FrdFile::Load(const std::string &frdPath, FrdFile &frdFile)
{
    ...
    std::ifstream frd{frdPath, std::ios::binary};

    bool loadStatus = frdFile.SerializeIn(frd);
    frd.close();
...
void FrdFile::Save(const std::string &frdPath, FrdFile &frdFile)
{
    ...
    std::ofstream frd{frdPath, std::ios::binary};
...
bool FrdFile::SerializeIn(std::istream &ifstream)
{
...
void FrdFile::SerializeOut(std::ostream &ofstream)
{
    // Write FRD Header
