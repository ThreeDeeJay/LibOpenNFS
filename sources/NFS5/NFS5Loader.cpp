...
std::shared_ptr<Car> NFS5::LoadCar(const std::string& carBasePath)
{
    ...
    std::filesystem::path outputPath(decompressedCrpPath.str());
    ...
    /*for (std::filesystem::directory_iterator itr(car_out_path.str()); itr != std::filesystem::directory_iterator(); ++itr) {
...
void NFS5::DumpCrpTextures(const std::string& crpPath)
{
    ...
    for (auto& fshPart : fshParts)
    {
        ...
        std::filesystem::path outputPath(crpPath);
