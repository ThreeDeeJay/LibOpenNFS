...
{
    std::filesystem::path p(carVivPath);
    ...
    for (std::filesystem::directory_iterator itr(carOutPath.str()); itr != std::filesystem::directory_iterator(); ++itr)
    {
...
std::shared_ptr<NFS3_4_DATA::PS1::TRACK> NFS4PS1::LoadTrack(const std::string &trackGrpPath)
{
    ...
    std::filesystem::path p(trackGrpPath);
