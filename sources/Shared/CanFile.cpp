...
bool CanFile::Load(const std::string &canPath, CanFile &canFile)
{
    ...
    bool loadStatus = canFile.SerializeIn(can);
    can.close();
...
void CanFile::Save(const std::string &canPath, CanFile &canFile)
{
    ...
    canFile.SerializeOut(can);
}

bool CanFile::SerializeIn(std::istream &ifstream)
{
...
void CanFile::SerializeOut(std::ostream &ofstream)
{
